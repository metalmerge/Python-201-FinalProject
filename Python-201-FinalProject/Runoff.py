...
def find_tie(min, collection, candidate_amount, voter_amount):
	...
	if tied == remaining_amount:
		...
		for a in range(remaining_amount):
			if collection[a]["votes"] == voter_amount / 2:
...
def start_Runoff():
	...
	while (True):
		candidate_count = int(input("How many candiates? (8 is maximum allowed because too much typing)\n"))
	...
	for z in range(candidate_count):
		percentage_data.append({"name":candidates[z]["name"],"votes":candidates[z]["votes"]})
	...
	while(True):
		...
		check = winner_found(candidates,candidate_count, voter_count, percentage_data)
		if check == True:
			break
